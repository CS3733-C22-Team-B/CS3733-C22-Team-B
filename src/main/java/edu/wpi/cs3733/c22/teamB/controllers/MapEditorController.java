                        ...
                        public void handle(MouseEvent event) {
                            if (moveState) {
                                ...
                                locationDBI.updateNode(
                                        new Location(
                                                selectedPnt.getId(),
                                                (int) getMapX(event.getX()),
                                                (int) getMapY(event.getY()),
                                                temp.getFloor(),
                                                temp.getBuilding(),
                                                temp.getNodeType(),
                                                temp.getLongName(),
                            ...
                            if (moveState) {
                                ...
                                double dist =
                                        calculateDistanceBetweenPoints(
                                                tempLoc.getXcoord(), tempLoc.getYcoord(), event.getX(), event.getY());
                                System.out.println(dist);
                                MedicalEquipment temp = medicalDBI.getNode(selectedImg.getId());
                                System.out.println(selectedImg.getId());
                                medicalDBI.updateNode(
                                        new MedicalEquipment(
                                                temp.getEquipmentID(),
                                                temp.getEquipmentName(),
                                                temp.getEquipmentType(),
    ...
    public Location getClosetLocation(double x, double y) {
        ...
        for (Node child : anchorPane.getChildren()) {
            if (child instanceof Circle) {
                double dist =
                        calculateDistanceBetweenPoints(
                                ((Circle) child).getCenterX(), ((Circle) child).getCenterY(), x, y);
                if (dist < distance) {
